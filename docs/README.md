...
That one way in is also how the platform itself travels. The coupler is designed as a kind of super app: OSDK adapted to run as an app on other operating systems — multiplatform kernel coupling and virtualization — so the Drayker platform can be mounted inside other environments, including super apps, instead of each one needing its own integration.
